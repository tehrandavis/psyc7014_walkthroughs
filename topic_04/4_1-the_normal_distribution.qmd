...
In the second half of this vignette I'll outline use cases for several of the scenarios that we will encounter this semester, offering tips and code templates for how to address each scenario. 

Before we start, let's generate a `normal_sample` of 60 observations that we know will be approximately normal and a positively skewed distribution `skewed_sample`. The code to create both is below:

```{r}
set.seed(1234) # set the seed to ensure we get the same results
...
# set the seed to ensure we get the same results
set.seed(1234) 

# create skewed distribution with mean 25, sd = 5, skew = 1.8, kurt = 4
skewed_sample <- 
...
I can't say it enough, one of the first things to do is plot your data. On of the first plots you should generate is a plot of the distribution of your variables as they fit within your design (more on this in Part 2). Your friends here are the **histogram plot** and the **QQ-plot**.

### Histogram plot

A histogram plot is simply a plot of the distribution of score. Typically this takes the form of the number (or raw count) of scores within a number of preset bin size, but can also take the form of the density, or proportion of scores. You may elect to create a density plot to overlay with a normal curve (a least using `ggplot`) a la the example from Section 3.1. 

I'll revisit the basics here, but the last 2 plot examples here are most useful to you.
...
Now we can apply it to any of our vectors for the rest of the session. For example, with `skewed_sample`:
