# From Hause Lin (https://gist.github.com/hauselin)
...
## My code
withinSummary <- function (data = NULL, measurevar, betweenvars = NULL, withinvars = NULL, 
                           idvar = NULL, na.rm = TRUE, conf.interval = 0.95) {
  ...
  # Combine the un-normed means with the normed results
  correctedData <- merge(datac, ndatac)
  output <- list(actualData,correctedData)
  names(output) <- c("Actual", "Corrected")
  #merged[, 1] <- as.numeric(as.character(merged[, 1]))
  #merged <- merged[order(merged[, 1]), ]
  return(output)
}
