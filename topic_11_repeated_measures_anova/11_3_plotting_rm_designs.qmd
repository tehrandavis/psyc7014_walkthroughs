...
If you are presenting a simple repeated measures design (e.g., very very factors × levels) then my general recommendation to combine the (uncorrected) mean and standard error bars with a violin plot or a rain cloud plot.

### Means with half violins

For now, let's just plot the means and standard error bars and add some nice half-violin plots (FWIW the good old fashion violins from earlier walkthroughs work well here as well). You'll need to call in the `ggdist` package for half violin plots.
...
You might even elect to have your conditions overlap (super level up):
...
That said, a slightly move involved method would be to to "correct" the standard error bars using a methods described by [Cousineau (2005)](https://doaj.org/article/dd17c4ccc4684c9da60c02f3864d09e6) and [Morey (2008)](https://doaj.org/article/011cd52c58b449c4bf169676ca704062).
...
Given the discussion above, we can plot with the corrected error bars. Let's first create a `plot_table` that contains the means and corrected values.
