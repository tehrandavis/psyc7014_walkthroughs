...
In this example, there is an implicit ordering where we assume `1 (Counting)` is the least level of processing and `5 (Intentional) is the highest. However at the moment this ordering is not taken into consideration by `R`. For example, let's say I wanted to consider levels of processing greater than Rhyming. If I wanted to only incude these levels in my data frame, I might:

```{r}
dataset %>%
  filter(GROUP_FACTOR > "Rhyming")
```
but as you can see this does not give me the intended outcome.

In order to fix this I can submit my labeled data to a second mutation:

```{r}
dataset <- 
  dataset %>% 
  mutate(GROUP_FACTOR_ORDERED = factor(GROUP_FACTOR, ordered = T)
  )

dataset
```

You'll notice that the new `GROUP_FACTOR_ORDERED` column contains the heading `<ord>`. And now:

```{r}
dataset %>%
  filter(GROUP_FACTOR_ORDERED > "Rhyming")
```

FWIW there is a question of whether this is truly appropriate given the analysis, BUT let's for a moment assume that we wanted to perserve the ordinal nature of the data, this is how you would do so.
