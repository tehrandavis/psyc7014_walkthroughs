...
model_aov <- lm(Time~Group_fct, data = dataset)
...
# get effect size
rstatix::eta_squared(morphine_mdl)
```


So we see here that we have: $F(4,35)=27.33,p<.001,\eta^2=.76$

### More complete anova tables

FWIW, in the example above we ran one function for the anova table and another for the effect size. R has many packages that will combine the two of them for you, including the `sjstats` package. For example `sjstats::anova_stats()` will give you a nice table with the ANOVA and effect size information, BUT it's probably a little too comprehensive. It give you a lot of information that you may not need, but is good if you elect to report more exotic effect sizes (e.g., omega squared, epsilonsq, etc.):

```{r}
sjstats::anova_stats(morphine_mdl)
```

For our purposes, `rstatix::anova_test()` provides a cleaner alternative. It gives you the ANOVA table, and effect sizes. It also gives you the option to specify the type of ANOVA that you want to run (e.g., Type II, Type III, etc., see Field text). For example:

```{r}
rstatix::anova_test(morphine_mdl, 
                    effect.size = "ges", # general eta squared for one-way ANOVA
                    type = 2) # type 2 is the default for R

#FWIW type=3 is the default for SPSS, see ?rstatix::anova_test for more info
```
...
```{r}
morphine_mdl <- lm(Time~Group_fct,data = dataset) # from above
...
## Model-based comparisions using estimated marginal means
